...
export default function useFetchCinemaList() {
  const [cinemaList, setCinemaList] = useState(
    store.getState().CinemaReducer.get('cinemaList')
  );

  // 让请求在redux中发，保证view层只处理ui层，如果先进的是search页面
  useEffect(() => {
    if (!store.getState().CinemaReducer.get('cinemaList').length) {
    ...
    const unsubscribe = store.subscribe(() => {
      setCinemaList(store.getState().CinemaReducer.get('cinemaList'));
      console.log(store.getState().CinemaReducer.get('cinemaList'), '-=-');
    });
    return () => {
      unsubscribe();
    };
